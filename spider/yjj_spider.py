...
from config import DATA_DIR, DOWNLOAD_YJJ_DIR, MAX_PAGES_YJJ
            ...
            for page_num in range(1, MAX_PAGES_YJJ + 1):
                ...
                for i, li in enumerate(lis):
                    ...
                    try:
                        ...
                        # 发布文号【无文号用发布时间代替】
                        fbwh_el = await li.query_selector("a")
                        fbwh_raw = await fbwh_el.inner_text() if fbwh_el else "" 
                        match = re.search(r'（(20\d{2}年第\d+号)）', fbwh_raw)
                        fbwh = match.group(1) if match else f"发布时间:{fbsj}" # 无发布文号用发布时间代替
