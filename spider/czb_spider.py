...
async def run_czb_spider():
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.goto(
                "http://gss.mof.gov.cn/gzdt/zhengcefabu/index.htm",
            ...
            for page_num in range(1, MAX_PAGES_CZB + 1):
                ...
                for i, li in enumerate(lis):
                    ...
                    try:
                        ...
                        attachments = await detail_page.query_selector_all(
                            "a[href*='.pdf'], a[href*='.doc'], a[href*='.docx'], a[href*='.xls'], a[href*='.xlsx'], a[href*='.zip'], a[href*='.wps']"
                        )
                        if attachments:
                                ...
                                if file_href and any(
                                    ...
                                    if file_href.startswith(
                                        ...
                                        if "mof.gov.cn" in full_url:
                                            file_full_url = (
                                                "https://www.mof.gov.cn" + file_href
                                            )
                                        else:
                                            file_full_url = (
                                                "http://gss.mof.gov.cn" + file_href
                                            )
                                    else:
                                        # 相对路径
                                    ...
                                    save_path = os.path.join(
                                        DOWNLOAD_CZB_DIR, file_name
                                    )
                                    await download_file(
                                        file_full_url, save_path, referer=full_url
                                    )  # 使用正确的URL和referer
                ...
                if page_num < MAX_PAGES_CZB:
                    next_button = await page.query_selector(
                        ".pagerji span:nth-last-child(2) > a"
                    )
