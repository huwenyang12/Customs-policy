...
from config import DATA_DIR, DOWNLOAD_GXB_DIR
from logger import log_info, log_error
from utils.file_utils import clean_filename, download_file

MAX_PAGES_GXB = 1 # 默认为1

async def run_gxb_spider():

    # 读取Excel文档，判断去重条件
    excel_path = os.path.join(DOWNLOAD_GXB_DIR, "工信部.xlsx")
    json_path = os.path.join(DATA_DIR, "工信部.json")
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.goto("https://www.miit.gov.cn/", wait_until="networkidle")
            await page.wait_for_timeout(6000)

            sum = 0  # 计数器
            for page_num in range(1, MAX_PAGES_GXB + 1):
                log_info(f"\n正在抓取第 {page_num} 页")
                
                lis = await page.query_selector_all('.tabbox-bds2 > div:nth-child(1) > ul > li')
                ...
                for i, li in enumerate(lis):
                    ...
                    try:
                        # 标题
                        title_el = await li.query_selector("p > a")
                        title = await title_el.get_attribute("title") if title_el else ""

                        # 发布时间
                        fbsj_el = await li.query_selector("p > span")
                        ...
                        png_path = os.path.join(DOWNLOAD_GXB_DIR, pdf_filename + ".png")
                        ...
                        pdf_path = os.path.join(DOWNLOAD_GXB_DIR, pdf_filename + ".pdf")
                        ...
                        attachments = await detail_page.query_selector_all("#con_con > p > a")
                        if attachments:
                            ...
                            try:
                                ...
                                if file_href and any(file_href.endswith(ext) for ext in ['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.zip', '.wps']):
                                    if file_href.startswith("/"):
                                        file_href = "https://www.miit.gov.cn" + file_href  # 工信部_补充href地址
                                    ...
                                    save_path = os.path.join(DOWNLOAD_GXB_DIR, file_name)
                                    await download_file(file_href, save_path, referer=href)  # 工信部文件下载
                        ...
                        zip_path = os.path.join(DOWNLOAD_GXB_DIR, zip_filename)
                        zip_file_count = 0
...
async def get_new_records_only():
    """只返回新增的记录"""
    # ... 在run_gxb_spider函数中，你可以直接返回new_records
    pass